...
if(isset($_GET['del_user'])){
    ...
    if($uid != $_SESSION['usuario_id']){
        $conexion->query("DELETE FROM usuario_sistema WHERE id=$uid");
    }
    echo "<script>window.location.href='?operacion=configuracion';</script>";
}
...
<div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap: 2rem;">
    ...
    <div class="card" style="padding:0; border:none; box-shadow:var(--shadow-lg);">
        <div style="padding: 2rem; border-bottom:1px solid var(--border); display:flex; justify-content:space-between; align-items:center;">
             <h3 style="margin:0; font-size:1.2rem; color:var(--text-main);">👥 Equipo</h3>
            <?php if($_SESSION['usuario_rol'] == 'admin'): ?>
                <a href="?operacion=insertar&tabla=usuario_sistema" class="btn btn-primary btn-sm">+ Nuevo</a>
            <?php endif; ?>
        </div>
        ...
        <div style="padding:0; max-height:400px; overflow-y:auto;">
            <table style="width:100%;">
                <tbody>
                    <?php
                    $res_users = $conexion->query("SELECT * FROM usuario_sistema ORDER BY rol ASC");
                    while($u = $res_users->fetch_assoc()):
                        $nombre = !empty($u['nombre_completo']) ? $u['nombre_completo'] : $u['usuario'];
                        $bg = ($u['rol']=='admin') ? '#6366f1' : '#94a3b8';
                        $initials = strtoupper(substr($nombre, 0, 2));
                    ?>
                    <tr style="border-bottom:1px solid var(--border);">
                        <td style="padding:1rem 2rem;">
                            <div style="display:flex; align-items:center; gap:12px;">
                                <div style="width:36px; height:36px; background:<?= $bg ?>; color:white; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:0.8rem; font-weight:bold;">
                                    <?= $initials ?>
                                </div>
                                <div>
                                    <div style="font-weight:600; color:var(--text-main); font-size:0.95rem;"><?= htmlspecialchars($nombre) ?></div>
                                    <div style="font-size:0.75rem; color:var(--text-muted); opacity:0.8;"><?= $u['rol'] ?></div>
                                </div>
                            </div>
                        </td>
                        <td style="text-align:right; padding:1rem 2rem;">
                             <?php if($u['id'] != $_SESSION['usuario_id']): ?>
                                <a href="?operacion=configuracion&del_user=<?= $u['id'] ?>" style="color:var(--danger);" onclick="return confirm('Eliminar?');">✕</a>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <?php endwhile; ?>
